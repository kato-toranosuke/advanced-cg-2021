...
public:
	...
	{
		double current_time;	//!< 最新の計測時間
		double total_time;		//!< 計測された時間をすべて足したもの
		double max_time;		//!< 計測された時間の中で最大のもの
		int count;				//!< 計測された回数
		int idx;				//!< 計測のインデックス(Splitで追加された順)
		rxTimeAndCount() : current_time(0.0), total_time(0.0), max_time(0.0), count(0), idx(0){}
	};
...
private:
	...
	rxTimeAndCount m_AllTime; //!< 計測時間(ResetTimeからStopTimeまで)
...
public:
	...
	void ClearTime(void)
	{
		for(RXMAPTC::iterator it = m_TimeMap.begin(); it != m_TimeMap.end(); ++it){
			...
			it->second.total_time = 0.0;
			it->second.max_time = 0.0;
			it->second.count = 0;
			//it->second.idx = -1;
		}
		m_AllTime.current_time = 0.0;
		m_AllTime.total_time = 0.0;
		m_AllTime.max_time = 0.0;
		m_AllTime.count = 0;
	}
	...
	{
		m_Tmr.Stop();
		m_AllTime.current_time += m_Tmr.GetTime(0);
		m_AllTime.total_time += m_AllTime.current_time;
		if(m_AllTime.current_time > m_AllTime.max_time) m_AllTime.max_time = m_AllTime.current_time;
		m_AllTime.count++;
	}
	...
	void Split(const string &cmnt)
	{
		...
		m_Tmr.Stop();
		if(i == m_TimeMap.end()){
			m_TimeMap[cmnt].current_time = m_Tmr.GetTime(0);
			m_TimeMap[cmnt].total_time = m_Tmr.GetTime(0);
			m_TimeMap[cmnt].max_time = m_Tmr.GetTime(0);
			...
			m_TimeMap[cmnt].current_time = m_Tmr.GetTime(0);
			m_TimeMap[cmnt].total_time += m_Tmr.GetTime(0);
			if(m_Tmr.GetTime(0) > m_TimeMap[cmnt].max_time) m_TimeMap[cmnt].max_time = m_Tmr.GetTime(0);
	...
	double GetTotalTime(void)
	{
		if(m_TimeMap.empty()){
			...
			double total = 0.0;
			for(RXMAPTC::iterator it = m_TimeMap.begin(); it != m_TimeMap.end(); ++it){
				total += it->second.total_time/it->second.count;
			}
	...
	/*!
	 * 現在の計測された時間(全Splitの合計)の取得
	 */
	double GetAllTime(void)
	{
		return m_AllTime.current_time;
	}

	double GetFPS(void)
	{
		double avg_time = m_AllTime.total_time/(double)(m_AllTime.count);
		return 1.0/avg_time;
	}

	void GetFPSString(char *str)
	{
		double avg_time = m_AllTime.total_time/(double)(m_AllTime.count);
		double current_fps = 1.0/m_AllTime.current_time;
		double avg_fps = 1.0/avg_time;
		sprintf(str, "%d step, %3.1f fps - avg %3.1f fps (%5.1f ms)", m_AllTime.count, current_fps, avg_fps, avg_time*1000);
	}
	...
	void Print(void)
	{
		...
		for(RXMAPTC::iterator it = m_TimeMap.begin(); it != m_TimeMap.end(); ++it){
			...
			cout << it->first << spc << " : " << (it->second.total_time/it->second.count) << "[s]" << endl;
		}
		cout.unsetf(ios::fixed);
		cout.precision(cur_p);
	...
	void PrintToString(string &str)
	{
		...
		for(int i = 0; i < (int)m_TimeMap.size(); ++i){
			...
			ss << it->first << spc << " : " << (it->second.total_time/it->second.count) << "[s]\n";
		}
